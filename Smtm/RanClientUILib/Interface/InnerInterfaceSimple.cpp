...
#include "ItemCompound.h" //add item compound by CN Dev
...
void CInnerInterface::SetTargetInfo ( STARGETID sTargetID )
{
	...
	if ( pCOPY->GetCrow() == CROW_MOB || pCOPY->GetCrow() == CROW_SUMMON )
	{
		...
		int fPercent3 =  (float(sHP.dwNow) / float(sHP.dwMax)) * 100;
				...
				float fPercent2 = float(sHP.dwNow) / float(sHP.dwMax);
				GLCharacter* pCharacter = GLGaeaClient::GetInstance().GetCharacter();

			PCROWDATA pCrowData = GLCrowDataMan::GetInstance().GetCrowData ( pCrow->m_sNativeID );
			if ( !pCrowData ) return;
				if( pCrowData->IsTowerMP() )
				{
					if( pCharacter->m_wSchool == 0 )
					{
						m_pTowerHPDisplay->SetTowerHPSG ( fPercent2 );
					}
					else if ( pCharacter->m_wSchool == 1 )
					{
						m_pTowerHPDisplay->SetTowerHPMP ( fPercent2 );
					}
					else if ( pCharacter->m_wSchool == 2 )
					{
						m_pTowerHPDisplay->SetTowerHPPHX ( fPercent2 );
					}
					
				}
				else if ( pCrowData->IsTowerPHX()  )
				{
					if( pCharacter->m_wSchool == 0 )
					{
						m_pTowerHPDisplay->SetTowerHPSG ( fPercent2 );
		...
		D3DXVECTOR3 vScreen = DxViewPort::GetInstance().ComputeVec3Project ( &vPos, NULL );	
		const UIRECT& rcOriginPos2 = m_pTowerHPDisplay->GetGlobalPos ();
		int nPosX2 = (int)( floor(vScreen.x - ( rcOriginPos2.sizeX * 0.5f )));
		int nPosY2 = (int)( vScreen.y - rcOriginPos2.sizeY );

		if ( m_pTowerHPDisplay )
		{
			m_pTowerHPDisplay->SetGlobalPos ( D3DXVECTOR2 ( float ( nPosX2 ), float ( nPosY2 ) ) );
		}

		if ( m_pCrowTargetInfo )	
		{
			m_pCrowTargetInfo->SetTargetInfo ( sHP.dwNow, sHP.dwMax, fPercent3, pCrow->GETLEVEL(), strName);
			
			if(  pCrowData->IsTowerSG() || pCrowData->IsTowerMP() || pCrowData->IsTowerPHX()  )
			{
				ShowGroupBottom ( TOWER_HP_DISPLAY ); 
				HideGroup ( TARGETINFO_DISPLAY );
				HideGroup ( CROW_TARGET_INFO );

				if ( m_pNameDisplayMan )	
					m_pNameDisplayMan->VisibleTargetInfoCtrl ( sTargetID.emCrow, sTargetID.dwID, FALSE );
			}
...
void CInnerInterface::SchoolWarStarted( NET_MSG_GENERIC* nmg )
{
	...
	if ( pNetMsg->nTIME==0 )
		m_pTowerDownDisplay->START ( 8 );
		m_pTowerDownDisplay->START ( 9 );
		ShowGroupTop ( TOWER_DOWN_DISPLAY );
}
void CInnerInterface::SchoolWarEnded( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_SCHOOL_WARS_END_BRD *pNetMsg = (GLMSG::SNET_SCHOOL_WARS_END_BRD *)nmg;
		...
		m_pTowerDownDisplay->START ( 8 );
		m_pTowerDownDisplay->START ( 10 );
		ShowGroupTop ( TOWER_DOWN_DISPLAY );
}
void CInnerInterface::TowerFinalSG()
{
	GLCharacter* pCharacter = GLGaeaClient::GetInstance().GetCharacter ();
	m_pTowerFinalDisplay->START ( 1 );
	if( pCharacter->m_wSchool == 0 )
	{
		m_pTowerFinalDisplay->START ( 4 );
	}
	ShowGroupTop ( TOWER_FINAL_DISPLAY );
}
void CInnerInterface::TowerFinalMP()
{
	GLCharacter* pCharacter = GLGaeaClient::GetInstance().GetCharacter ();
	m_pTowerFinalDisplay->START ( 3 );
	if( pCharacter->m_wSchool == 1 )
	{
		m_pTowerFinalDisplay->START ( 4 );
	}
	ShowGroupTop ( TOWER_FINAL_DISPLAY );
}
void CInnerInterface::TowerFinalPHX()
{
	GLCharacter* pCharacter = GLGaeaClient::GetInstance().GetCharacter ();

	m_pTowerFinalDisplay->START ( 2 );
	if ( pCharacter->m_wSchool == 2 )
	{
		m_pTowerFinalDisplay->START ( 4 );
	}
	ShowGroupTop ( TOWER_FINAL_DISPLAY );
}
void CInnerInterface::TowerDown ( NET_MSG_GENERIC* nmg )
{
	GLMSG::SNET_SW_TOWERDOWN_BRD *pNetMsg = (GLMSG::SNET_SW_TOWERDOWN_BRD *) nmg; 
	
	if ( pNetMsg->bOn == true )
	{
		if ( pNetMsg->emSchool == 0 )
		{
			if ( pNetMsg->emFB ==  EMTOWER_DESTROYED_SG || pNetMsg->emFB ==  EMTOWER_DESTROYED_SGMP 
				|| pNetMsg->emFB == EMTOWER_DESTROYED_SGPHX )
			{
				//THERMAL
				if( pNetMsg->emFB == EMTOWER_DESTROYED_SG )
				{
					m_pCTFInfoDisplay->ShowSGTher();
				}
				else if ( pNetMsg->emFB ==  EMTOWER_DESTROYED_SGMP )
				{ 
					m_pCTFInfoDisplay->ShowSGFaci();
				}
				else if ( pNetMsg->emFB == EMTOWER_DESTROYED_SGPHX )
				{
					m_pCTFInfoDisplay->ShowSGNucl();
				}

				m_pTowerDownDisplay->START ( 2 );
				m_pTowerDownDisplay->START ( 5 );
				m_pTowerDownDisplay->START ( 1 );

				ShowGroupTop ( TOWER_DOWN_DISPLAY );
			}
		}
		else if ( pNetMsg->emSchool == 1 )
		{
			if ( pNetMsg->emFB ==  EMTOWER_DESTROYED_MPSG || pNetMsg->emFB ==  EMTOWER_DESTROYED_MP 
				|| pNetMsg->emFB == EMTOWER_DESTROYED_MPPHX )
			{
				//FACILITY
				if( pNetMsg->emFB == EMTOWER_DESTROYED_MPSG )
				{
					m_pCTFInfoDisplay->ShowMPTher();
				}
				else if ( pNetMsg->emFB ==  EMTOWER_DESTROYED_MP )
				{ 
					m_pCTFInfoDisplay->ShowMPFaci();
				}
				else if ( pNetMsg->emFB == EMTOWER_DESTROYED_MPPHX )
				{
					m_pCTFInfoDisplay->ShowMPNucl();
				}

				m_pTowerDownDisplay->START ( 3 );
				m_pTowerDownDisplay->START ( 6 );
				m_pTowerDownDisplay->START ( 1 );
				
				ShowGroupTop ( TOWER_DOWN_DISPLAY );
			}
		}
		else if ( pNetMsg->emSchool == 2 )
		{
			if ( pNetMsg->emFB ==  EMTOWER_DESTROYED_PHXSG || pNetMsg->emFB ==  EMTOWER_DESTROYED_PHXMP 
				|| pNetMsg->emFB == EMTOWER_DESTROYED_PHX )
			{
				//NUCLEAR
				if( pNetMsg->emFB == EMTOWER_DESTROYED_PHXSG )
				{
					m_pCTFInfoDisplay->ShowPHXTher();
				}
				else if ( pNetMsg->emFB ==  EMTOWER_DESTROYED_PHXMP )
				{ 
					m_pCTFInfoDisplay->ShowPHXFaci();
				}
				else if ( pNetMsg->emFB == EMTOWER_DESTROYED_PHX )
				{
					m_pCTFInfoDisplay->ShowPHXNucl();
				}

				m_pTowerDownDisplay->START ( 4 );
				m_pTowerDownDisplay->START ( 7 );
				m_pTowerDownDisplay->START ( 1 );
...
HRESULT CInnerInterface::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	}
	{
		...
		m_pCTFInfoDisplay->Create ( INFO_CTF_DISPLAY, "INFO_CTF_DISPLAY" );
		m_pCTFInfoDisplay->CreateSubControl();
		m_pCTFInfoDisplay->SetAlignFlag ( UI_FLAG_CENTER_X );
		RegisterControl ( m_pCTFInfoDisplay );
		ShowGroupTop ( INFO_CTF_DISPLAY  );
		HideGroup ( INFO_CTF_DISPLAY );
	}

	{
			m_pTowerFinalDisplay = new CTowerFinalDisplay;
			m_pTowerFinalDisplay->Create ( TOWER_FINAL_DISPLAY , "TOWER_FINAL_DISPLAY" );
			m_pTowerFinalDisplay->CreateSubControl ();
			RegisterControl ( m_pTowerFinalDisplay );
			ShowGroupTop ( TOWER_FINAL_DISPLAY );

	}

	{
		m_pTowerDownDisplay = new CTowerDownDisplay;
		m_pTowerDownDisplay->Create ( TOWER_DOWN_DISPLAY, "TOWER_DOWN_DISPLAY" );
		m_pTowerDownDisplay->CreateSubControl();
		RegisterControl ( m_pTowerDownDisplay );
		ShowGroupTop ( TOWER_DOWN_DISPLAY );
	}
	{
...
//add charinfoview
	{
		...
		m_pCharacterViewPropertiesWindow->CreateBaseWindowLightGray ( "2014_VIEWCHARACTER_WINDOW", (char*)ID2GAMEWORD("VIEW_CHARACTER_NAME_STATIC") );
	...
	{
			m_pTowerHPDisplay = new CTowerHPDisplay;
			m_pTowerHPDisplay->Create ( TOWER_HP_DISPLAY , "TOWER_HP_DISPLAY" );
			m_pTowerHPDisplay->CreateSubControl ();
			RegisterControl ( m_pTowerHPDisplay );
			ShowGroupBottom ( TOWER_HP_DISPLAY );
...
//#endif
...

	{
		...
		m_pCharacterWindow->CreateBaseWindowLightGrayUntitled ( "CHARACTER_WINDOW_MAIN" );
		m_pCharacterWindow->CreateSubControl ();
		m_pCharacterWindow->SetAlignFlag ( UI_FLAG_LEFT | UI_FLAG_TOP );
...

	{
		...
		m_pCharacterWindowAdditionalInfo->Create ( CHARACTER_ADDITIONAL_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pCharacterWindowAdditionalInfo->CreateBaseWindowLightGrayUntitled ( "CHARACTER_ADDITIONAL_WINDOW" );
...

	{
		...
		pSkillWindow->CreateBaseWindowLightGray ( "RNSKILL_WINDOW", (char*)ID2GAMEWORD("SKILL_NAME_STATIC") );
...

	{
		...
		m_pPartyWindow->CreateBaseWindowLightGray ( "PARTY_WINDOW_RENEWAL", (char*)ID2GAMEWORD("PARTY_NAME_STATIC") );
...
//		ShowGroupFocus ( MARKET_WINDOW );
	...
	{	//	인벤토리
		...
		m_pInventoryWindow->CreateBaseWindowLightGray ( "__INVENTORY_WINDOW", (char*)ID2GAMEWORD("INVENTORY_NAME_STATIC") );
...
//#endif
...

	{
		...
		m_pFriendWindow->CreateBaseWindowLightGray( "FRIEND_WINDOW", (char*)ID2GAMEWORD("FRIEND_WINDOW_NAME_STATIC") );
...

	{
		...
		pBikeColorCard->CreateBaseWindowLightGray( "HAIRCOLORCARD_WINDOW", NULL );
		...
		pItemColorCard->CreateBaseWindowLightGray( "HAIRCOLORCARD_WINDOW", NULL );
...
//		ShowGroupFocus ( VNGAINSYS_GAUGE_WINDOW );
	...
	{	//  소환수
		...
		m_pSummonWindow->CreateBaseWindowLightGray( "SUMMON_WINDOW", (char*)ID2GAMEWORD("SUMMON_WINDOW_STATIC",0 ) );
...
void CInnerInterface::UpdateClubBattleTime2( float fClubBattleTime2 )
{
	if( m_pMiniMap )
...
void CInnerInterface::UpdateSchoolWarBattleTime( float fSchoolWarsTime )
{
	if( m_pCTFInfoDisplay )
		m_pCTFInfoDisplay->UpdateSchoolWarTime( fSchoolWarsTime );
}
