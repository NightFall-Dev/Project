...
// #endif
	...
	else 
	{
		if ( UIKeyCheck::GetInstance()->Check( DIK_TAB, DXKEY_DOWN ) )
		{
			...
			if( emClass&GLCC_MAGICIAN_M || emClass&GLCC_MAGICIAN_W )
			{
				if (GLCONST_CHAR::bDUAL_WEAPON_MAGICIAN )
				{
				// 스킬 사용중으로 실패하면 Tab Key 예약
					if ( E_FAIL == GLGaeaClient::GetInstance().GetCharacter()->ReqSlotChange() )
						m_bTabReserve = true;
...
//#endif	
	...
	if ( pLand->m_bSchoolWars )
		{																
				...
				m_pCTFInfoDisplay->ShowCTFInfo();
				m_pCTFInfoDisplay->ShowNULLFaci();
				m_pCTFInfoDisplay->ShowNULLTher();
				m_pCTFInfoDisplay->ShowNULLNulc();
		}
	else
	{
		m_pCTFInfoDisplay->HideCTFInfo();
