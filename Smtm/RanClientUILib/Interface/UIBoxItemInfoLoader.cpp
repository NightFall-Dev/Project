	...
	void AddInfoSkillItem ( const SITEMCUSTOM &sItemCustom )
	{
		...
		if ( pSkill )
		{
			//	기본 정보
			{
				...
				//add class "MAGICIAN"
				if ( pSkill->m_sLEARN.dwCLASS & GLCC_MAGICIAN_M )	strText.AppendFormat ( "%s", COMMENT::CHARCLASS[14].c_str() );
				if ( pSkill->m_sLEARN.dwCLASS & GLCC_MAGICIAN_W )	strText.AppendFormat ( "%s", COMMENT::CHARCLASS[15].c_str() );
