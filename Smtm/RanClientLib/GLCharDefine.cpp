...
EMCHARINDEX CharClassToIndex ( const EMCHARCLASS emClass )
{
	...
	case GLCC_ASSASIN_W:	return GLCI_ASSASIN_W;
		//add class "MAGICIAN"
	case GLCC_MAGICIAN_M:	return GLCI_MAGICIAN_M;
	case GLCC_MAGICIAN_W:	return GLCI_MAGICIAN_W;
		//
	};
...
EMCHARCLASS CharClassGetSex ( const EMCHARINDEX emIndex ) //add abl
{
	...
	case GLCI_SCIENCE_M:
	case GLCI_ASSASIN_M:
	case GLCI_MAGICIAN_M:	return GLCC_MAN;
	case GLCI_FIGHTER_W:
	case GLCI_ARMS_W:
	case GLCI_ARCHER_W:
	case GLCI_SPIRIT_W:
	...
	case GLCI_SCIENCE_W:
	case GLCI_ASSASIN_W:
	case GLCI_MAGICIAN_W:	return GLCC_WOMAN;
	};
...
EMCHARCLASS CharIndexToClass ( const EMCHARINDEX emIndex )
{
	...
	case GLCI_ASSASIN_W:	return GLCC_ASSASIN_W;
		//add class "MAGICIAN"
	case GLCI_MAGICIAN_M:	return GLCC_MAGICIAN_M;
	case GLCI_MAGICIAN_W:	return GLCC_MAGICIAN_W;
		//
	};
...
EMCHARCLASS CharClassToSex ( const EMCHARCLASS emClass )
{
	...
	case GLCC_ASSASIN_W:	return GLCC_ASSASIN_M;
		//add class "MAGICIAN"
	case GLCC_MAGICIAN_M:	return GLCC_MAGICIAN_W;
	case GLCC_MAGICIAN_W:	return GLCC_MAGICIAN_M;
		//
	};
...
EMCHARINDEX CharIndexToSex ( const EMCHARINDEX emIndex )
{
	...
	case GLCI_ASSASIN_W:	return GLCI_ASSASIN_M;
		//add class "MAGICIAN"
	case GLCI_MAGICIAN_M:	return GLCI_MAGICIAN_W;
	case GLCI_MAGICIAN_W:	return GLCI_MAGICIAN_M;
		//
	};
...
VOID GetCharSkillClassIndex( const EMCHARCLASS emClass, WORD & wBeginIndex, WORD & wEndIndex )
{
	...
	case GLCC_ASSASIN_W:
		...
		break;
		//add class "MAGICIAN"
	case GLCC_MAGICIAN_M:
	case GLCC_MAGICIAN_W:
		{
...
namespace COMMENT
{
	...
	std::string CHARCLASS[GLCI_NUM_2012] =
	{
		"Brawler(M)",
		"Swordsman(M)",
		"Archer(W)",
		...
		"Archer(M)",
		"Shaman(M)",
		"Science(M)",//add class "SCIENCE"
		"Science(W)",
		"Assassin(M)",//add class "ASSASSIN"
		"Assassin(W)",
		"Magician(M)",//add class "MAGICIAN"
		"Magician(W)",
	};
	...
	std::string CHARCOMMENT[GLCI_NUM_2012] = 
	{
		"",
		"",
		"",
