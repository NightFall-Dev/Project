		...
		case EMACT_SLOT_DRUG:
			{
				...
				if ( pITEM->sDrugOp.emDrug == ITEM_DRUG_ANTI_POTS )
				{
					if ( m_pOwner->ISANTIPOTS() )
					{
						m_pOwner->ReqInvenDrug ( pInvenItem->wPosX, pInvenItem->wPosY );
						return;
					}
				}
				if ( pITEM->sDrugOp.emDrug == ITEM_DRUG_ANTI_SKILL )
				{
					if ( m_pOwner->ISANTISKILL() )
					{
						m_pOwner->ReqInvenDrug ( pInvenItem->wPosX, pInvenItem->wPosY );
						return;
					}
				}
				if ( pITEM->sDrugOp.emDrug == ITEM_DRUG_ANTI_STUN )
				{
					if ( m_pOwner->ISANTISTUN() )
					{
						m_pOwner->ReqInvenDrug ( pInvenItem->wPosX, pInvenItem->wPosY );
						return;
					}
