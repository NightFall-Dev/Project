...
void GLChar::MsgSendUpdateState ( bool bparty, bool bconfront, bool bview )
{
	...
	// 성향이벤트 이면 주변에도 
	if ( bview || bGuidBattleMap || bClubDeathMatch || bSchoolWars || bRoyalRumble || bSCHOOL_FREEPK || bBRIGHTEVENT )
	{
...
inline HRESULT GLChar::MsgAttack ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pTARGET->GetCrow()==CROW_PC )
	{
		...
		if ( pCHAR )
		{
			...
			if ( !(bGuidBattleMap || bClubDeathMatch || bSchoolWars || bSCHOOL_FREEPK || bFreePKMap || bBRIGHTEVENT || IsConflictTarget(pTARGET) || bClubBattle ) )
			{
