...
//		cFILE.getflag( "fDISPRICE", 1, 1, fDISPRICE );
		...
		for ( i=0; i<GLCI_NUM_2012; ++i )
			cFILE.getflag( "fCHAR_HEIGHT", i+1, GLCI_NUM_2012, fCHAR_HEIGHT[i] );
		...
		cFILE.getflag( "bDUAL_WEAPON_MAGICIAN", 1, 1, bDUAL_WEAPON_MAGICIAN );
		...
		cFILE.getflag( "bCreateMagician", 1, 1, bCreateMagician );
		...
		//add class "SCIENCE", "ASSASSIN", "MAGICIAN"
		const char* szSETFILE[GLCI_NUM_2012] = 
		{
			"FIGHTER_M.SETFILE",
			"ARMS_M.SETFILE",
			"ARCHER_W.SETFILE",
		...
		for ( i=0; i<wSCHOOLNUM; ++i )
		{
			...
			for ( WORD j=0; j<GLCI_NUM_2012; ++j )
			{
				...
				//add class "SCIENCE", "ASSASSIN", "MAGICIAN"
				cFILE.getflag( strKEY.GetString(), 1+j, GLCI_NUM_2012, strCLASS_INIT[i][j] );	
		...
		//add class
		const char* szSKILFILE[GLCI_NUM_2012] = 
		{
			"FIGHTER_M.SKINFILE",
			"ARMS_M.SKINFILE",
			"ARCHER_W.SKINFILE",
