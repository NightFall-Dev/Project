...
BOOL GLGaeaServer::EntryLand( DWORD dwGaeaID, DWORD dwGateID, BOOL bInstantMap, SNATIVEID sMapID /*= NATIVEID_NULL()*/ )
{
	...
	else pInGate = pInGateMan->FindLandGate ( pOutGate->GetToGateID() );
...
BOOL GLGaeaServer::GMCtrolClassEvent   ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_CLASS_EVENT *pNetMsg )
{
	...
	if( pNetMsg->dwClassIndex > GLCC_ALL_2012 ) return FALSE;

	//bool bClassEvent[GLCI_NUM_NEWSEX];
	//for( BYTE i = 0; i < GLCI_NUM_NEWSEX; i++ )
	//add class "SCIENCE", "ASSASSIN", "MAGICIAN"
	bool bClassEvent[GLCI_NUM_2012];
	for( BYTE i = 0; i < GLCI_NUM_2012; i++ )
	...
	//add class "MAGICIAN"
	if( pNetMsg->dwClassIndex&GLCC_MAGICIAN_M )  bClassEvent[14] = TRUE;
	if( pNetMsg->dwClassIndex&GLCC_MAGICIAN_W )  bClassEvent[15] = TRUE;
	//
...
/*BOOL GLGaeaServer::SetItemLink( GLMSG::SNET_ITEMLINK_ITEM_BRD *pNetMsg )
{
	...
	if( pLandMan )
	{
		GLMSG::SNET_ITEMLINK_ITEM_BRD *NetMsg = (GLMSG::SNET_ITEMLINK_ITEM_BRD *) pNetMsg;
		NetMsg->Data = pNetMsg->Data;
		NetMsg->strMessage = pNetMsg->strMessage;
		GLCHARNODE* pCharNode = pLandMan->m_GlobPCList.m_pHead;
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
