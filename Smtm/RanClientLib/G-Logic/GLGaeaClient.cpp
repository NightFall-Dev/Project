...
#include "../../RanClientUILib/Interface/CTFInfoDisplay.h"
	...
	case NET_MSG_GM_CLASS_EVENT:
		{
			...
			switch(pNetMsgFB->dwClassIndex)
			{
				...
				case GLCI_ASSASIN_W:  strcpy( szClassChar, "ASSASIN_W:" ); break;
					//add class "MAGICIAN"
				case GLCI_MAGICIAN_M:  strcpy( szClassChar, "MAGICIAN_M:" ); break;
				case GLCI_MAGICIAN_W:  strcpy( szClassChar, "MAGICIAN_W:" ); break;

				case GLCI_NUM_2012:  strcpy( szClassChar, "ALL:" ); break;
					//
			}
	...
	case NET_MSG_GCTRL_SCHOOL_WARS_START_BRD:
		{
			...
			if ( pNetMsg->nTIME==0 )
			{
				...
				CCTFInfoDisplay * pCTFInfoDisplay = CInnerInterface::GetInstance().GetCTFInfoDisplay();
				if( pCTFInfoDisplay ) pCTFInfoDisplay->StartSchoolWarTime();
			}
			else
			{
	...
	case NET_MSG_GCTRL_SCHOOL_WARS_END_BRD:
		{
			...
			CInnerInterface::GetInstance().SchoolWarEnded( nmg );
			m_bSchoolWars = false;
			m_fSchoolWarsTimer = 0.0f;

			CCTFInfoDisplay * pCTFInfoDisplay = CInnerInterface::GetInstance().GetCTFInfoDisplay();
			if( pCTFInfoDisplay ) pCTFInfoDisplay->EndSchoolWarTime();
			pCTFInfoDisplay->HideCTFInfo();
			GetCharacter()->DEL_PLAYHOSTILE_ALL();
		}
		break;
	...
	case NET_MSG_GCTRL_ROYAL_RUMBLE_END_BRD:
		{
			...
			CInnerInterface::GetInstance().PrintConsoleText ( "%s Dungeon I has ended", pNetMsg->szName );
		...
		case NET_MSG_GCTRL_TOWERDOWN_BRD:
		{
			...
			if( pNetMsg->bOn )
			{
				//NUCLEAR
						...
						if( pNetMsg->emFB == EMTOWER_DESTROYED_PHXMP )	TDCOUNTPHXFAC();
						if( pNetMsg->emFB == EMTOWER_DESTROYED_PHX )	TDCOUNTPHXNUC();
						if( pNetMsg->emFB == EMTOWER_DESTROYED_PHXSG )  TDCOUNTPHXTHE();
						TDTotalPHX();
						switch( m_dwTDTotalPHX )
						{
						case 1:
							{
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("PH Captured 1 Tower") );
						...
						case 2:
							{
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().TowerFinalPHX();
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("PH Captured 2 Towers") );
							}
							break;
						case 3:
							{
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("PH Captured All Towers, SchoolWar has ended.") );
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().SchoolWarEnded( nmg );
								GLGaeaClient::ReqSchoolWarsEnd();
								TDResetSG();
								TDResetMP();
								TDResetPHX();
				...
				//FACILITY
						if ( pNetMsg->emFB == EMTOWER_DESTROYED_MPSG ) TDCOUNTMPTHE();
						if ( pNetMsg->emFB == EMTOWER_DESTROYED_MP )  TDCOUNTMPFAC();
						if ( pNetMsg->emFB == EMTOWER_DESTROYED_MPPHX )  TDCOUNTMPNUC();

						TDTotalMP();
						switch( m_dwTDTotalMP )
						{
						case 1:
							{
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("MP Captured 1 Tower") );
							}
							break;
						case 2:
							{
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().TowerFinalMP();
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("MP Captured 2 Towers") );
							}
							break;
						case 3:
							{
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("MP Captured All Towers, SchoolWar has ended.") );
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().SchoolWarEnded( nmg );
								GLGaeaClient::ReqSchoolWarsEnd();
								TDResetSG();
				...
				//THERMAL
						if ( pNetMsg->emFB == EMTOWER_DESTROYED_SG )  TDCOUNTSGTHE();
						if ( pNetMsg->emFB == EMTOWER_DESTROYED_SGMP )  TDCOUNTSGFAC();
						if ( pNetMsg->emFB == EMTOWER_DESTROYED_SGPHX )  TDCOUNTSGNUC();

						TDTotalSG();
						switch( m_dwTDTotalSG )
						{
						case 1:
							{
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("SG Captured 1 Tower") );
							}
							break;
						case 2:
							{
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().TowerFinalSG();
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("SG Capture 2 Towers") );
							}
							break;
						case 3:
							{
								CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::DARKGRAY,("SG Captured All Towers, SchoolWar has ended.") );
								CInnerInterface::GetInstance().TowerDown( nmg );
								CInnerInterface::GetInstance().SchoolWarEnded( nmg );
								GLGaeaClient::ReqSchoolWarsEnd();
								TDResetSG();
