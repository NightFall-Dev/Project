...
HRESULT GLChar::SaveLastCall ()
{
	...
	if ( !m_pLandMan->m_bGuidBattleMap && !m_pLandMan->m_bClubDeathMatchMap && !m_pLandMan->m_bSchoolWarsMap && !m_pLandMan->m_bRoyalRumbleMap )
	{
...
void GLChar::AddPlayHostile ( DWORD dwCHARID, BOOL bBAD, BOOL bClubBattle )
{
	...
	if ( !(bBRIGHTEVENT||bSCHOOL_FREEPK||bGuidBattleMap||bClubDeathMatch||bSchoolWars) )
	{
		//	Note : 적대 행위자 등록. ( pk )
