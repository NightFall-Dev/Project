...
BOOL GLAgentServer::MsgGmViewWorkEvent ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	for( int i = 0; i < 3; i++ )
	{
		...
		//add class
		if( m_WorkEvent.class_Event[i].dwClassIndex == GLCC_ALL_2012 )
		{
			...
			//add class "MAGICIAN"
			if( m_WorkEvent.class_Event[i].dwClassIndex&GLCC_MAGICIAN_M )
			{
				strcat( szTempChar, "MAGICIAN_M," );
			}
			if( m_WorkEvent.class_Event[i].dwClassIndex&GLCC_MAGICIAN_W )
			{
				strcat( szTempChar, "MAGICIAN_W," );
			}
		}
		szTempChar[strlen(szTempChar)-1] = '\0';
